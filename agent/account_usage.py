...
def _fetch_zai_account_usage(api_key: Optional[str]) -> Optional[AccountUsageSnapshot]:
    ...
    # Z.AI returns a "limits" array with TIME_LIMIT and TOKENS_LIMIT entries
    ...
    for lim in limits:
        ...
        if lim_type == "TOKENS_LIMIT" and pct is not None:
            ...
            # Z.AI uses epoch milliseconds (not seconds)
    ...
    *,
    ...
    if normalized not in {"openai-codex", "anthropic", "openrouter", "zai", "nous"}:
        ...
        if "api.z.ai" in _host or "open.bigmodel.cn" in _host:
            ...
            # resolve the explicit zai provider key from config.  The key may
            # live under the canonical name or the custom provider alias.
            if not api_key:
                try:
